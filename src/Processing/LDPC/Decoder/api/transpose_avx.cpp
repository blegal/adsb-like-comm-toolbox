...
#undef  LOAD_AND_DECIDE
#undef  LOAD_SIMD_FX
#undef  STORE_SIMD_FX
#define LOAD_AND_DECIDE(a) _mm256_and_si256(_mm256_cmpgt_epi8(_mm256_loadu_si256((const __m256i*)(a)),mone),mask)
//#define LOAD_AND_DECIDE(a) _mm256_loadu_si256( (const __m256i*)(a) )
#define LOAD_SIMD_FX       _mm256_loadu_si256
#define STORE_SIMD_FX(a,b) _mm256_storeu_si256((__m256i*)(a), b)


void uchar_itranspose_2_avx(__m256i *src, __m256i *dst, int N_BIN)
{
    const int vect_loop     = (N_BIN / 32);
    const __m256i mask      = _mm256_set1_epi8( 0x01 );
    const __m256i mone      = _mm256_set1_epi8(   -1 );
    const char *p_input  = (const char*)src;
    char *p_output = (char*)dst;

    for(int q = 0; q < vect_loop; q += 1){
        __m256i a = LOAD_AND_DECIDE(p_input +   0);
        __m256i b = LOAD_AND_DECIDE(p_input +  32);
        __m256i c = LOAD_AND_DECIDE(p_input +  64);
        __m256i d = LOAD_AND_DECIDE(p_input +  96);
        __m256i e = LOAD_AND_DECIDE(p_input + 128);
        __m256i f = LOAD_AND_DECIDE(p_input + 160);
        __m256i g = LOAD_AND_DECIDE(p_input + 192);
        __m256i h = LOAD_AND_DECIDE(p_input + 224);
        __m256i i = LOAD_AND_DECIDE(p_input + 256);
        __m256i j = LOAD_AND_DECIDE(p_input + 288);
        __m256i k = LOAD_AND_DECIDE(p_input + 320);
        __m256i l = LOAD_AND_DECIDE(p_input + 352);
        __m256i m = LOAD_AND_DECIDE(p_input + 384);
        __m256i n = LOAD_AND_DECIDE(p_input + 416);
        __m256i o = LOAD_AND_DECIDE(p_input + 448);
        __m256i p = LOAD_AND_DECIDE(p_input + 480);
        ...
        __m256i A               = LOAD_AND_DECIDE(p_input + 16*32);
        __m256i B               = LOAD_AND_DECIDE(p_input + 17*32);
        __m256i C               = LOAD_AND_DECIDE(p_input + 18*32);
        __m256i D               = LOAD_AND_DECIDE(p_input + 19*32);
        __m256i E               = LOAD_AND_DECIDE(p_input + 20*32);
        __m256i F               = LOAD_AND_DECIDE(p_input + 21*32);
        __m256i G               = LOAD_AND_DECIDE(p_input + 22*32);
        __m256i H               = LOAD_AND_DECIDE(p_input + 23*32);
        __m256i I               = LOAD_AND_DECIDE(p_input + 24*32);
        __m256i J               = LOAD_AND_DECIDE(p_input + 25*32);
        __m256i K               = LOAD_AND_DECIDE(p_input + 26*32);
        __m256i L               = LOAD_AND_DECIDE(p_input + 27*32);
        __m256i M               = LOAD_AND_DECIDE(p_input + 28*32);
        __m256i N               = LOAD_AND_DECIDE(p_input + 29*32);
        __m256i O               = LOAD_AND_DECIDE(p_input + 30*32);
        __m256i P               = LOAD_AND_DECIDE(p_input + 31*32);
        ...
        STORE_SIMD_FX(p_output +  0 * N_BIN, a_0_p_0_A_0_P_0);
        STORE_SIMD_FX(p_output +  1 * N_BIN, a_2_p_2_A_2_P_2);
        STORE_SIMD_FX(p_output +  2 * N_BIN, a_4_p_4_A_4_P_4);
        STORE_SIMD_FX(p_output +  3 * N_BIN, a_6_p_6_A_6_P_6);
        STORE_SIMD_FX(p_output +  4 * N_BIN, a_8_p_8_A_8_P_8);
        STORE_SIMD_FX(p_output +  5 * N_BIN, a_10_p_10_A_10_P_12);
        STORE_SIMD_FX(p_output +  6 * N_BIN, a_12_p_12_A_12_P_12);
        STORE_SIMD_FX(p_output +  7 * N_BIN, a_14_p_14_A_14_P_14);
        STORE_SIMD_FX(p_output +  8 * N_BIN, a_16_p_16_A_16_P_16);
        STORE_SIMD_FX(p_output +  9 * N_BIN, a_18_p_18_A_18_P_18);
        STORE_SIMD_FX(p_output + 10 * N_BIN, a_20_p_20_A_20_P_20);
        STORE_SIMD_FX(p_output + 11 * N_BIN, a_22_p_22_A_22_P_22);
        STORE_SIMD_FX(p_output + 12 * N_BIN, a_24_p_24_A_24_P_24);
        STORE_SIMD_FX(p_output + 13 * N_BIN, a_26_p_26_A_26_P_26);
        STORE_SIMD_FX(p_output + 14 * N_BIN, a_28_p_28_A_28_P_28);
        STORE_SIMD_FX(p_output + 15 * N_BIN, a_30_p_30_A_30_P_30);

        STORE_SIMD_FX(p_output + 16 * N_BIN, a_1_p_1_A_1_P_1);
        STORE_SIMD_FX(p_output + 17 * N_BIN, a_3_p_3_A_3_P_3);
        STORE_SIMD_FX(p_output + 18 * N_BIN, a_5_p_5_A_5_P_5);
        STORE_SIMD_FX(p_output + 19 * N_BIN, a_7_p_7_A_7_P_7);
        STORE_SIMD_FX(p_output + 20 * N_BIN, a_9_p_9_A_9_P_9);
        STORE_SIMD_FX(p_output + 21 * N_BIN, a_11_p_11_A_11_P_12);
        STORE_SIMD_FX(p_output + 22 * N_BIN, a_13_p_13_A_13_P_13);
        STORE_SIMD_FX(p_output + 23 * N_BIN, a_15_p_15_A_15_P_15);
        STORE_SIMD_FX(p_output + 24 * N_BIN, a_17_p_17_A_17_P_17);
        STORE_SIMD_FX(p_output + 25 * N_BIN, a_19_p_19_A_19_P_19);
        STORE_SIMD_FX(p_output + 26 * N_BIN, a_21_p_21_A_21_P_21);
        STORE_SIMD_FX(p_output + 27 * N_BIN, a_23_p_23_A_23_P_23);
        STORE_SIMD_FX(p_output + 28 * N_BIN, a_25_p_25_A_25_P_25);
        STORE_SIMD_FX(p_output + 29 * N_BIN, a_27_p_27_A_27_P_27);
        STORE_SIMD_FX(p_output + 30 * N_BIN, a_29_p_29_A_29_P_29);
        STORE_SIMD_FX(p_output + 31 * N_BIN, a_31_p_31_A_31_P_31);
    ...
    //
    // PARTIE SCALAIRE DU PROCESSING SI CELA EST NECESSAIRE
    // (EN FONCTION DE LA TAILLE DE LA TRAME A ENTRELACER).
    //
    const int scal_loop  = 32 * vect_loop;
    if( scal_loop != N_BIN ){
        char* p_src = (char*) src;
        char* p_dst = (char*) dst;
        for (int i = scal_loop; i < N_BIN; i += 1) {
            for (int j = 0; j < 32; j += 1) {
                p_dst[j * N_BIN + i] = (p_src[32 * i + j] >= 0);
            }
        }
    }
}
