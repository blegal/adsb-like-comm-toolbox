#include "Detector_NEON_Accu.hpp"

#if defined(__ARM_NEON)
    #include <arm_neon.h>
#endif

Detector_NEON_Accu::Detector_NEON_Accu() : Detector(1)
{

}


void Detector_NEON_Accu::execute(std::vector<float>* iBuffer, std::vector<float>* oBuffer)
{
#if defined(__ARM_NEON)
    ...
    {
        const float32x4_t dt = vld1q_f32(ptr_i + k);
        accu += (dt * dt);
    }
...
#else
    printf("Le decodeur instancie pour realiser le traitement est le Detector_NEON_Accu, mais ce dernier");
