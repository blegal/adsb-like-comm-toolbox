...
struct BMP {
    ...
    uint8_t get_pixel_r(const uint32_t x0, const uint32_t y0) {
        if (x0 > (uint32_t)bmp_info_header.width || y0 > (uint32_t)bmp_info_header.height) {
            throw std::runtime_error("The point is outside the image boundaries!");
        }

        const uint32_t channels = bmp_info_header.bit_count / 8;
        return data[channels * (y0 * bmp_info_header.width + x0) + 2];
    }


    uint8_t get_pixel_g(const uint32_t x0, const uint32_t y0) {
        if (x0 > (uint32_t)bmp_info_header.width || y0 > (uint32_t)bmp_info_header.height) {
            throw std::runtime_error("The point is outside the image boundaries!");
        }
    ...
    uint8_t get_pixel_b(const uint32_t x0, const uint32_t y0) {
        if (x0 > (uint32_t)bmp_info_header.width || y0 > (uint32_t)bmp_info_header.height) {
            throw std::runtime_error("The point is outside the image boundaries!");
        }
