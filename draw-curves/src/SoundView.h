...
{
public:
    SoundView(std::vector<int16_t>& buffer );

    ~SoundView();

    void setYOffset(int _offset);

    void update(std::vector<int16_t>& buffer);

    void update_plot();

    void update_plot(sf::VertexArray* m_vertices, std::vector<int16_t>& buffer, int yOffset);
